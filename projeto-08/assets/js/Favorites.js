...
export class Favorites {
  ...
  async add(username){
    try{
      ...
      if(userExists) {
        throw new Error(`Usuário já cadastrado: ${username}`)
      }

      const githubUser = await GithubUser.search(username)

      if(githubUser.login === undefined || githubUser.name === null){
...
export class FavoritesView extends Favorites {
  ...
  update() {
    ...
    this.entries.forEach(user => {
      ...
      row.querySelector('.remove').onclick = () => {
        const isOk = confirm('Deseja realemnte remover o usuário?')
        if(isOk){
          this.delete(user)
        }
      }
  ...
  creatNotFav(){
    ...
    <p class="not-fav">Nenhum favorito ainda</p>
    </div>
  ...
  createRow() {
    ...
    tr.innerHTML = `
      <td class="user">
      <img src="https://avatars.githubusercontent.com/u/81930401?v=4" alt="">
      <a href="https://github.com/ViniciusOliver-stack" target="_blank">
        <p>Vinicius Oliveira</p>
        <span>/ViniciusOliver-stack</span>
      </a>
      </td>
      <td class="repositories">12</td>
      <td class="followers">150</td>
      <td>
